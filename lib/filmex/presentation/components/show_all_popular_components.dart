...
int numberOfPages=1;
class ShowAllPopularComponents extends StatelessWidget {
  const ShowAllPopularComponents({super.key});
        ...
        switch(state.popularState){
          ...
          case RequestState.loaded:
            return ListView.separated(
              shrinkWrap: true,
              itemCount: state.popularFilmex.length + 1, // Add 1 for the "Load More" button
              itemBuilder: (context, index) {
                if (index == state.popularFilmex.length) {
                  return Padding(
                    ...
                    child: Row(
                      children: [
                        IconButton(
                          ...
                          onPressed: () {
                            numberOfPages++;
                            AppConstance().navigateTo(context, ShowAllFilmex(pageNumbering: numberOfPages, nameScreen: AppString.popularFilmex,));
                          },
                        ),
                        const Spacer(),
          ...
          return SizedBox(
            height: 400,
            child: Text(state.popularMessage,style: const TextStyle(color: Colors.red,fontSize:25,fontWeight: FontWeight.w600),),
          );
        }
